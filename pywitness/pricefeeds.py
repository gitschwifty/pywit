...
class PriceFeed():
    ...
    def get_pair(self, pair):
        ...
        if pair == "SU":
            ...
            avg = self.get_avg(su)
            logstr = "Final STEEM to USD Price: {:.3f} USD per STEEM".format(avg)
            ...
            #multiply and return
            btu = bt * tu
            logstr = "Bitcoin to USD Price (Proxied through USDT): {:.4f} USD per BTC".format(btu)
            self.log.log(logstr, 2)
            ...
            #list prices off
            avg = self.get_avg(p)
            logstr = "Average Bitcoin to USDT Price: {:.4f} USDT per BTC".format(avg)
            self.log.log(logstr, 2)
            ...
            avg = self.get_avg(p)
            logstr = "Average USDT to USD Price: {:.4f} USD per USDT".format(avg)
            self.log.log(logstr, 2)
    ...
    def get_prices(self, pair):
        ...
        if pair == 1:
            ...
            try:
                ...
                r = requests.get('https://bittrex.com/api/v1.1/public/getmarketsummary?market=BTC-STEEM', timeout=30)
                ...
                r = requests.get('https://api.binance.com/api/v1/ticker/24hr', timeout=30)
                if r.status_code == 200:
                ...
                r = requests.get('https://poloniex.com/public?command=returnTicker', timeout=30)
                if r.status_code == 200:
                    d = r.json()['BTC_STEEM']
                    prices.append(float(d['last']))
            ...
            try:
                self.log.log("Querying Bittrex", 2)
                r = requests.get('https://bittrex.com/api/v1.1/public/getmarketsummary?market=USD-BTC', timeout=30)
                if r.status_code == 200:
                    d = r.json()['result'][0]
                    prices.append(float(d['Last']))
            ...
            try:
                ...
                self.log.log("Querying Bittrex", 2)
                r = requests.get('https://bittrex.com/api/v1.1/public/getmarketsummary?market=USDT-BTC', timeout=30)
                if r.status_code == 200:
                    d = r.json()['result'][0]
                    prices.append(float(d['Last']))
            ...
            try:
                self.log.log("Querying Bittrex", 2)
                r = requests.get('https://bittrex.com/api/v1.1/public/getmarketsummary?market=USD-USDT', timeout=30)
                if r.status_code == 200:
                    d = r.json()['result'][0]
                    prices.append(float(d['Last']))
                ...
                r = requests.get('https://api.kraken.com/0/public/Ticker?pair=USDTZUSD', timeout=30)
                if r.status_code == 200:
    ...
    def do_feed(self, pnow):
        ...
        if per > self.min_spread or self.pub_today == 24 or pnow:
            if per < 25 and self.last_price != 0:
                ...
                logstr = "Percent difference {:.2f}% greater than 25%, not publishing.".format(per)
                self.log.log(logstr, 1)
        else:
            logstr = "Percent difference {:.2f}% lower than minimum spread {:.2f}%, not publishing.".format(per, self.min_spread)
            self.log.log(logstr, 1)
            self.pub_today += 1
        self.log.pop_func()
    ...
    def run_feeds(self, slptime=6000, spread=2.0, pnow=False):
        ...
        logstr = "Running price feeds with minimum spread {:.2f} and sleep time {:.2f} minutes.".format(self.min_spread, (slptime / 60))
        self.log.log(logstr, 1)
        while True:
            try:
                self.do_feed(pnow)
